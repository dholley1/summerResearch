...
void setup() {
  ...
  background(50);
  rectMode(CENTER);
  input = loadImage("IMG_0912.JPG");
  image(input, 0, 0, 200, 200);
  for(int i = 0; i < POPULATION; i++) {
    float deltaX = random(10, 30);
    // starting positions, dx, dy, brush length, speed
    float[] vs = {0, 0, random(2, 10), random(2, 10), deltaX, deltaX, random(5, 15)};
