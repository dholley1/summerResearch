float findDist(color c1, color c2) {
  ...
  // Convert the RGB colors into the LAB colors.
  ...
  // Get the distance between the LAB colors, using distance formula.
  float distance = sqrt(pow(L1-L2, 2) + pow(a1-a2, 2) + pow(b1-b2, 2));
  return distance;
}

public float[] RGB2LAB(float R, float G, float B) {
  /* Given a RGB color, returns the LAB color of it. */
...
/*
CITE: https://forum.processing.org/two/discussion/6903/unexpected-result-in-color-conversion-from-lab-to-rgb
*/

public float[] LAB2RGB(float L, float a, float b) {
  /* Given a LAB color, returns the RGB color of it. */
